...
### Product-Knowledge Lane B (per project)

**Lane B = product / domain knowledge** (vision, principles, positioning, feature-fit criteria) — distinct
from **Lane A** (AOS methodology + LOD-tiered engineering docs). Lane B is **not** on the LOD ladder. See
**Part 4a** for the full convention. These paths are binding for any project that authors product knowledge:

| Data | Canonical Path | Format | Owner |
|------|---------------|--------|-------|
| Product docs (long-form) | `documentation/docs-system/08-PRODUCT/` | Directory | Domain lead architect (per `core/definition.yaml`) |
| Product-knowledge file (long-form) | `documentation/docs-system/08-PRODUCT/PRODUCT_<topic>.md` | Markdown | Domain lead architect |
| Product-knowledge snippet (thin, agent-read) | `_aos/context/PRODUCT_<topic>.md` | Markdown | Architecture agent |

`08` is the **reserved PRODUCT ordinal** in the `documentation/docs-system/NN-TOPIC/` numbered-directory
pattern (aligns with the existing `08-PRODUCT/` directory; do NOT create a conflicting root-level
`08-PRODUCT/`). The `PRODUCT_*.md` filename pattern (`PRODUCT_<topic>.md`, e.g. `PRODUCT_VISION.md`,
`PRODUCT_POSITIONING.md`, `PRODUCT_FEATURE_FIT.md`) marks a file as Lane B by name in both homes.
...
| Data | Canonical Path | Format | Owner |
|------|---------------|--------|-------|
| Completed WP artifacts | `_archive/[WP-ID]/` | Directory | team_120 custodian; exec at closure by closing orchestrator (team_100 / team_110-ADR045) via `/AOS_archive`; git by team_60 — D-191auth (team_191 dissolved 2026-07-05) |
| Archive manifest | `_archive/[WP-ID]/ARCHIVE_MANIFEST.md` | Markdown | team_120 custodian (as above) |
...
## Part 4a: Product-Knowledge Lane B convention

Product/domain knowledge and methodology/engineering documentation are **two distinct lanes**. Naming the
lane tells an agent where a document lives, how it is named, and whether it sits on the LOD ladder.

| Lane | What it holds | Home | LOD-tiered? |
|------|---------------|------|-------------|
| **Lane A — methodology / engineering** | LOD-tiered specs (LOD300–500), `spec_ref`, DOC_CANON, architecture/API docs — "how it is built and what is true now". | `_aos/work_packages/` (LOD chain) + `documentation/docs-system/01-ARCHITECTURE`, `02-SERVER`, … | **Yes** — governed by `LOD_STANDARD_v5`. |
| **Lane B — product / domain knowledge** | Vision, principles, positioning, feature-fit criteria — "what this product is and why". | `documentation/docs-system/08-PRODUCT/` (long-form) + `_aos/context/PRODUCT_<topic>.md` (thin, agent-read) | **No** — Lane B is NOT on the LOD ladder; a `PRODUCT_*.md` file requires no LOD chain. |

**Convention (binding, fleet-wide):**

1. **Directory home.** Long-form product knowledge lives under the domain's
   `documentation/docs-system/08-PRODUCT/` directory. `08` is the reserved PRODUCT ordinal in the
   `documentation/docs-system/NN-TOPIC/` numbered-directory pattern (e.g. `01-ARCHITECTURE`, `02-SERVER`,
   `07-DESIGN`, `08-PRODUCT`). If a domain has no `documentation/docs-system/` tree yet, create it when
   first authoring Lane B content — do NOT invent a conflicting root-level `08-PRODUCT/`.
2. **File-naming pattern.** A Lane B file is named `PRODUCT_<topic>.md` (glob `PRODUCT_*.md`) — e.g.
   `PRODUCT_VISION.md`, `PRODUCT_POSITIONING.md`, `PRODUCT_FEATURE_FIT.md`. The `PRODUCT_` prefix marks the
   file as Lane B by name in either home.
3. **Activation-layer snippets.** Thin, agent-read product-knowledge snippets may also live in
   `_aos/context/` as `PRODUCT_<topic>.md`, alongside `PROJECT_CONTEXT.md`. Keep these thin (Part 1a: "one
   thin file every agent reads first … avoid duplicating full … product text"). **Deep/long-form** product
   docs go to the `08-PRODUCT/` directory home, not `_aos/context/`.
4. **Not LOD-tiered.** Lane B content is exempt from the LOD ladder — a validator does NOT expect a
   LOD200/400/500 chain for a `PRODUCT_*.md`. (Lane A remains fully LOD-governed.)
5. **This convention is defined once, here.** A spoke MAY point to its Lane B docs from
   `PROJECT_CONTEXT.md` (Part 1a `### Standards / SSOT`), but the *convention itself* is fleet-wide and is
   not re-defined in any spoke file. A domain lead architect (per `core/definition.yaml`) owns the Lane B
   content for that domain.

> **This convention records a pattern; it mandates no content.** No project is required to create Lane B
> docs. The convention says *where they go and how they are named* if and when a project authors product
> knowledge — keeping product knowledge from scattering across `_aos/context/`, `_COMMUNICATION/`, and
> ad-hoc directories.
...
| team_120³ | mandate only | mandate only | R | propagation | W (own dir) |
...
>
> ³ **team_120 propagation authority (D-191auth):** team_120 (Ambassador) inherits the `_aos/`
> propagation-write authority of the **dissolved team_191** (Git/Files — dissolved by team_00 on
> **2026-07-05**). Under the split: `_aos/` propagation + post-gate-archive **custodianship** → team_120;
> git/file operations (commit/push) → team_60; archival execution → API-mediated `archive.py` / `/AOS_archive`
> run by the closing orchestrator (team_100 hub-native; team_110 under ADR045). team_191 no longer appears in
> this table as an active writer.
...
15. **Completed WP artifacts archive to `_archive/`.** When a WP reaches status=COMPLETE (LOD500_LOCKED after L-GATE_VALIDATE PASS), all communication artifacts from `_COMMUNICATION/team_*/[WP-ID]/` MUST be moved to `_archive/[WP-ID]/`. Each archived WP directory MUST contain an `ARCHIVE_MANIFEST.md` listing: source paths, archive date, file count, archiving team. The archive procedure MUST include a misplaced artifact scan — detect WP-scoped files incorrectly placed at team root directories instead of WP subdirectories (violation of Iron Rule #12). `_archive/` is flat (one level of WP-ID directories, no nesting). Post-gate archiving is **custodian: team_120** (inheriting dissolved team_191's `_aos/` authority per D-191auth, 2026-07-05); **executed** at closure by the closing orchestrator (team_100 hub-native; team_110 under ADR045) via `/AOS_archive` (API-mediated `archive.py`), with the git commit/push by **team_60**; any team may execute under an explicit Team 00 mandate. No permanent deletions — only moves. `_COMMUNICATION/99-ARCHIVE/` is deprecated; `_archive/` at project root is the canonical archive path.
...
*Amended 2026-07-01 (v1.1.2 — Part 4a + Part 4 registry: Product-Knowledge Lane B convention (GCR-120-06) — `documentation/docs-system/08-PRODUCT/` + `_aos/context/PRODUCT_*.md`; NOT LOD-tiered; distinct from Lane A)*
