...
**Version:** 1.0.0 | **Date:** 2026-05-11 | **v5 update:** 2026-06-20 (added §0 one-command provisioning) · 2026-06-24 (added §0.1 agent presentation convention — always present team_00 the cd + provision command)
...
The AOS hub API at `http://100.125.98.56:8092` (waldhomeserver, Tailscale) requires all mutation
...
## §0 v5 Quick Path — One-Command Dev Provisioning (STANDARD as of 2026-06-20)

For routine dev/session provisioning against the **v5 server (`:8092`)**, use the single Mac-side
command `scripts/provision_actor_key.sh` instead of the manual §2–§4 issue→retrieve→export flow.
It wraps the same validated server-side `issue_actor_key.sh` / `retrieve_actor_key.sh` and adds
secure local delivery + auto-load. Verified live 2026-06-20 (team_100/00/90/190 → HTTP 200).

```bash
# Run once per team, as an approved issuer (team_00 or team_99):
AOS_ACTOR_TEAM_ID=team_00 bash scripts/provision_actor_key.sh team_100 team_00 team_90 team_190
```

What it does: issues each key on the v5 worktree → restarts `aos-api-v5` once → retrieves each key
over SSH only → writes `~/.aos/actor_keys.json` + `~/.aos/actor.env` (both `0600`) → wires `~/.zshrc`
to auto-source the default identity + `AOS_API_BASE=:8092` → verifies (authenticated inbox → 200).

**Result:** every NEW shell/session (any domain) is authenticated **and** pointed at v5, zero
per-session fuss. Switch the active team in a shell: `aos_actor team_190`. The key never appears in
chat/commit/log — it lives only in two `0600` files (this Mac + server `core/.env`) and travels only
over SSH.

Authority + security are unchanged from §1/§8/§9 (only team_00/team_99/team_100 may issue; only
team_00/team_99 may retrieve — so provision as team_00 or team_99). Rotation/revocation still use
§6/§7. **Tracked follow-up:** parameterize `issue/rotate/revoke_actor_key.sh` with
`AOS_API_SERVICE=aos-api-v5` so those also target the v5 unit directly.

### §0.1 Agent presentation convention (team_00 standing directive, 2026-06-24)

When ANY agent/session determines a new actor key must be generated — e.g. a non-issuer team hits a
`401 INVALID_ACTOR_KEY` because it has no key — it MUST present the issuer a **copy-paste-ready
command block that INCLUDES the `cd` to the worktree.** Never a prose description, and never an
attempt to self-issue (only `{team_00, team_99, team_100}` may issue — ADR040 / IR#12; only
`{team_00, team_99}` may retrieve). Canonical format (this exact shape, every time):

```bash
cd <worktree-abs-path>
AOS_ACTOR_TEAM_ID=team_00 bash scripts/provision_actor_key.sh <target_team> [<target_team> ...]
```

Then the one-line verify: `aos_actor <target_team> && msg_curl GET /api/system/health` → expect
`HTTP 200`. The agent SHOULD also file the §2 request artifact to `_COMMUNICATION/team_99/`, but the
ready-to-run block to team_00 is **mandatory** and is the primary deliverable.
...
4. Receiving team sets:
   ...
   export AOS_API_BASE=http://100.125.98.56:8092
   ```
...
AOS_API_BASE=http://100.125.98.56:8092
```
