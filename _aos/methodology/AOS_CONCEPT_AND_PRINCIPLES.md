...
- **Physical methodology snapshots** (each project carries its own governance copy — a physical copy or, under Model B/ADR054, a git-ignored local cache refreshed by sync)
...
2. **Physical lean-kit / governance cache:** `_aos/lean-kit/` (and the governance snapshots `_aos/governance/`, `_aos/methodology/`) are a physical copy **or** a git-ignored local cache refreshed by sync (Model B / ADR054) — **never a symlink**. The hub remains the audit SSoT; a tracked `_aos/AOS_GOVERNANCE_VERSION.yaml` stamp records the version each domain holds. One-directional flow (Rule #11) is preserved.
...
10. **Deploy verification by prod STATE (Iron Rule #16):** A deploy is GREEN only when prod state is asserted equal to the intended release — `curl <host>/health` `git_sha` **==** the pushed/expected commit SHA **AND** `readlink /opt/<project>/current` **==** the intended `/opt/<project>/releases/<tag>`. A git-hook / ssh **exit code**, a `"DEPLOY OK"`/`"DEPLOY GREEN"` **log line**, or a `200` **without** a `git_sha` compare is **never** sufficient (an old-but-healthy process also returns 200). Each host has exactly **one** authoritative deploy mechanism — a release-symlink flip (`ln -sfn … current`) with health-verify + auto-rollback; the bare-repo `post-receive` hook MUST **delegate** to it and MUST NOT run a parallel working-clone reset (`git reset --hard /data/projects/<x>`) the service never reads. Deploy-critical Model-B assets (port-canon cache et al.) are **seeded** into each release from the live host cache — never delivered by a VCS checkout, never un-ignored (DV-4 / ADR054 C-MB1 / `validate_aos.sh` Check 61 on the spoke). Every host is audited for the two-path drift **fingerprint** (a `/data/projects/<x>` working clone the hook writes vs an `/opt/<x>/current` served symlink the service reads) on initial deploy, after any hook/deploy-script change, and after any home-server migration. The health endpoint MUST expose `git_sha` (or an equivalent build id) for the rule to be checkable. Canon: `lean-kit/modules/12-home-server-infrastructure/deployment/DEPLOY_VERIFICATION_CANON_v1.0.0.md` (DV-1..DV-4; content v1.1.0). Architectural decision record: ADR056. Empirical origin: 2026-07-05 tiktrack silent-no-op-deploy drift (`git_sha` never changed while `deploy.log` said `DEPLOY OK`); same fingerprint confirmed on agros-insite + hub `aos-api-v5`.

11. **Orchestrator worktree isolation (Iron Rule #17):** Any session running fleet-mutating orchestration (`aos_sync_all.sh`, `propagate_governance.sh` standalone, `sync_definition_snapshots.sh` standalone, or any future hub→spoke propagation entrypoint) MUST run from a **dedicated isolated worktree** — never the shared primary hub checkout (`git worktree list`'s first/main entry). A script/canon fix authored mid-run MUST be committed to a branch + PR'd — NEVER left uncommitted in the shared primary checkout. Mechanically enforced (not just documented): `scripts/lib/aos_worktree_guard.sh`, sourced by all three entrypoints, refuses a mutating run (non-`--dry-run`) from the primary checkout unless the documented override `AOS_ALLOW_PRIMARY_CHECKOUT_SYNC=1` is set; `validate_aos.sh` Check 76 independently FAILs if the primary checkout ever carries uncommitted changes under `scripts/`, `lean-kit/`, `core/governance/`, `methodology/`, or `governance/directives/` — a second, DB-independent layer that catches a violation regardless of how it occurred. This closes the ADR052 worktree-isolation gap specifically for the orchestration path: `ensure_automation_worktree.sh` already created an isolated worktree but only for session-registration bookkeeping — the actual propagation/commit logic still ran against wherever the script was invoked from, giving a false sense of isolation. Architectural decision record: `ADR052_ADDENDUM_ORCHESTRATOR_WORKTREE_ENFORCEMENT_v1.0.0.md`. Empirical origin: 2026-07-15 — a team_100 propagation session authored a genuine sync-script fix but left it uncommitted in the shared primary checkout while a concurrent team_120 session was landing engine-canon PRs there; team_120 preserved the fix out-of-band (PR #44) before any loss occurred, but the near-miss (and ADR052's own Context section, which had already logged this exact failure mode twice before) is what this rule closes.
