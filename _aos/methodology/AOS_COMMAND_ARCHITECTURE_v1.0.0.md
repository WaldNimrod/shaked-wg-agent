...
API endpoint: `{METHOD} {HUB_API_BASE}/api/{path}` (substitute `HUB_API_BASE` from env; default `http://127.0.0.1:8092`)
...
| `/AOS_dispatch` | `POST {HUB_API_BASE}/api/validation/cross-engine/exec` (+ `POST /api/messaging/send`) | ✓ (v5) |
| `/AOS_help` | `GET {HUB_API_BASE}/api/commands` (future) | — |
| `/AOS_mail`, `/AOS_send`, `/AOS_server` | Wrappers — no API | — |

`HUB_API_BASE` defaults to `http://127.0.0.1:8092`; override via `AOS_API_BASE` env when hub runs on waldhomeserver (V321).
