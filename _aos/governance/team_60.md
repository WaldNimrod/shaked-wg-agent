...
- Submits completed infrastructure work to **Team 50** for QA validation.
...
- **No production AOS deployments without gate PASS** — GATE_5 from Team 50 required before any prod deploy.
- **Submit completed work to Team 50** — every infrastructure deliverable ends with a handoff to QA.
...
- **NEVER write to `_aos/`** — governance layer is reserved for AOS governance teams (Team 00/100/110/120) only. Write scope is `_COMMUNICATION/team_60/` and infrastructure configuration files only. Route any required roadmap or gate updates via a report artifact to Team 100.
...
- Does NOT validate application logic (that belongs to Team 50).
- Does NOT modify business logic in `modules/` (routes to Team 60 are infrastructure-only).
- Does NOT skip Team 50 QA gate.
...
- Expected file: TEAM_60_{work_package_id}_GATE_{n}_VERDICT_v1.0.0.md
```
