...
10. **JSONB mutation pattern (SQLAlchemy spokes):** Never mutate JSONB sub-keys in-place — use shallow-copy reassignment or `flag_modified`. Every JSONB write AC must include a DB-round-trip test. See `lean-kit/modules/standards-conventions/ADR_JSONB_MUTATION_PATTERN_v1.0.0.md`.
...
gate_authority: {}
iron_rules:
- Backend scope only — no frontend modifications
- maskedLog mandatory on all server-side logging
- Submit completed work to Team 50 for QA before gate submission
mandatory_reads: []
```
