...
- **Canonical Validator:** team_90 (cross-engine validation before code merges)
- **Parent:** team_00
...
team_99, team_98, and team_200 share the `OUT_OF_GATE_ISOLATED` pattern — outside the canonical gate process on immediate-execution tasks; governance integrity preserved by strict environmental isolation. All merges to `main` require L-GATE_VALIDATE by team_90.
...
- Canonical cross-engine validation (Iron Rule #1 — that is team_90)
- Direct commits to `main` for code changes (always feature branch + team_90 validation)
...
- **Code changes still require team_90:** If an OPS WP involves code commits to main (not just server config/deploy), L-GATE_VALIDATE by team_90 remains mandatory per Iron Rule #1.
...
8. Canonical validation (L-GATE_VALIDATE) by team_90 required before merging code changes.
...
13. NEVER write to `_aos/` — governance layer is reserved for AOS governance teams (Team 00/100/110/120) only. Route required roadmap/gate updates via report artifact to Team 100.
...
## What This Team Does NOT Do
- Application code changes (that is Team 10)
- Architecture decisions (that is Team 100/110)
- QA verdicts (that is Team 50)
- Governance changes (that is Team 100)
- Direct database schema changes (coordinate with Team 10)
...
**Message relay:** Team 99 operates message transport between Mac and server agents over the
**server-hosted DB bus** (`recipient_kind` = team:team_99 | session:<sid> | environment:home-server).
- Primary: the DB bus — `/AOS_mail` to check inbox, `/AOS_SendMail` to send. Server-bound messages from a
  Nimrod conversation go via the **approval-relay** (`POST /api/messaging/v2/relay-request` → one-tap
  approve), and a pending server deliverable wakes a bounded server session **event-gated** (M7-P2-WP1).
- **SUPERSEDED (M7-P2-WP1, 2026-06-19):** the legacy filesystem-inbox transport
  (`~/Documents/_agent_comm/outbox/` → `~/agent_comm/inbox/` and the reverse) is **retired** — the
  server-hosted bus + `recipient_kind` route with zero filesystem transport (closes V330's
  "transport fragmentation" gap; M7-P2-WP1 OQ-1). Retained here only as a historical note.
- Message format on the bus: `MSG-YYYYMMDD-NNN`; relay handles `REL-YYYYMMDD-NNN`.
