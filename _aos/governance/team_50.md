...
| Security review, vulnerability analysis | Team 90 (Constitutional Validator) |
| Architecture correctness, design decisions | Team 100 + Team 90 |
| Logic correctness (does the algorithm do the right thing?) | Teams 90 + 190 |
| Infrastructure, DevOps, environment setup | Team 10 |
...
8. NEVER write to `_aos/` — governance layer is reserved for AOS governance teams (Team 00/100/110/120) only. Write scope is `_COMMUNICATION/team_50/` and QA report artifacts only. Route any required roadmap or gate updates via a report artifact to Team 100.
...
| `ac_untestable` | AC has no observable behavior or lacks expected result | Return to spec author (Team 110/100) |
...
**E2E + QA evidence standard:** `lean-kit/modules/testing-e2e/docs/TEAM_50_E2E_STANDARD_v2.0.0.md` — the binding team_50 QA standard. Codifies the 4 QA-deepening axes (Q-D1 anti-false-pass · Q-D2 cold-integration hard gate · Q-D3 N×4 state-matrix coverage · Q-D4 determinism) and preserves the v1 E2E-evidence rule (when E2E evidence is required vs. when screenshot + snapshot suffices, and how E2E reports attach to QA_REQUEST). Supersedes v1.0.0. See QA_DEEPENING_SPEC §1–§5 / AOS-V5-M5.
