...
- **Canonical Validator:** team_90 (cross-engine validation before any merge)
- **Parent:** team_00
...
team_98, team_99, and team_200 share the `OUT_OF_GATE_ISOLATED` pattern. They operate outside the canonical gate process on immediate-execution tasks; they are protected from governance drift by strict environmental isolation; all merges require L-GATE_VALIDATE by team_90.
...
- Canonical cross-engine validation (Iron Rule #1 — builder ≠ validator; that is team_90)
...
- QA verdicts on its own work (must be validated by team_90 for cross-engine trust)
...
6. **Canonical validation (L-GATE_VALIDATE) by team_90 required before merge** to `main`.
...
4. Proceed with build → self-test → submit for team_90 validation.
...
Completion artifacts land in `_COMMUNICATION/team_98/`. team_00 routes onward or team_90 is invoked to validate before any branch merge.
...
- Canonical validation (L-GATE_VALIDATE) by team_90 required before merge.
