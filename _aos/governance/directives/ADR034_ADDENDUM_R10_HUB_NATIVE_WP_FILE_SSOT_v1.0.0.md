...
## v5 Update — WP-ID validation centralized (2026-06-16 · M2-W8)

AOS v5 introduces a **new canonical WP-ID format** and centralizes all WP-ID validation. This note
records that change for traceability; it does **not** weaken or supersede the R10 ruling above.

1. **`core/wp_id.py` is the single source of truth** for WP-ID formats (anti-drift / P9), replacing the
   regex copies that previously drifted across `portfolio.py`, `machine.py`, `seed.py`, and
   `gate2_db_helpers.py`.
2. **v5 canon — `M{n}-P{n}-WP{n}-{essence-slug}`** (e.g. `M2-P1-WP7-enum-ssot`; L15 / CA1 §6) is a
   recognized first-class production format, built server-side by the cockpit C3 create flow (no
   free-form names). It joins the legacy `S{NNN}-P{NNN}-WP{NNN}` (spoke / L2), `NB-Vn-WP-*` (content
   substrate), and ULID bootstrap formats. The "API accepts only `SNNN-PNNN-WPNNN` and `NB-Vn-WP-*`"
   wording in the Problem Statement reflects the pre-v5 DB schema; `core/wp_id.py` now also recognizes
   the v5 canon.
3. **R10's ruling is unchanged.** Hub-native `AOS-V*-WP-*` IDs remain file-canonical (no DB row; `POST`
   returns HTTP 400 `INVALID_WP_ID_FORMAT` by design) and are intentionally NOT accepted by
   `core/wp_id.py`. v5 milestone WPs in the `M{n}-P{n}-WP{n}-slug` format are likewise managed as
   **file-SSoT** when no hub DB row exists (`metadata.yaml` + git commit = audit record; ADR034 R9/R10).
