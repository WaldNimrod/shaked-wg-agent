...
1. Resolve `AOS_API_BASE` using the three-tier chain (see §15): Tier 1 = explicit `AOS_API_BASE` env var; Tier 2 = `AOS_V3_PUBLIC_API_BASE` from `core/.env`; Tier 3 = `http://127.0.0.1:8092` (correct on waldhomeserver; hits Mac legacy stub — see Rule 4).
...
4. **Rule 4 — HTTP 410 (Mac legacy stub signal):** *(added in v1.4.0)* HTTP 410 is a **redirect directive**, NOT a connection failure or a fallback condition. The `aos_legacy_stub.py` process (installed at v4.0.0 GA on Mac, port 8090) deliberately returns HTTP 410 to prevent Mac sessions from silently writing to a non-canonical DB. On HTTP 410: set `API_ONLINE=0`, emit an **actionable** error naming the canonical server URL (`http://100.125.98.56:8092`), and proceed to file fallback as a last resort (same as timeout path). **Do NOT treat 410 as silent success of the fallback path.** The user MUST see a message directing them to set `AOS_API_BASE=http://100.125.98.56:8092`.
5. **If connection refused / timeout** → API is offline → proceed to file fallback AND emit:
   `⚠ API offline — using file fallback. Canonical API: http://100.125.98.56:8092 (waldhomeserver, Tailscale). Set AOS_API_BASE=http://100.125.98.56:8092`
...
⚠ API auth unavailable (ACTOR_KEY_NOT_CONFIGURED) — falling back to file delivery.
  Cause: server AOS_ACTOR_KEYS has no entry for {team_id}.
...
> **⚠ v5-CUTOVER OVERRIDE (2026-06-28, AOS-V5-WP-EXTERNAL-AUDIT-REMEDIATION WS4/FIX4 — team_120 diagnosis).**
> The canonical AOS API is **`aos-api-v5` on port `:8092`** (waldhomeserver `http://100.125.98.56:8092`, local
> `http://127.0.0.1:8092`). **`:8090` is the RETIRED v4 stub everywhere** — it returns `{"code":"AOS_V4_RETIRED"}`
> on the server and HTTP 410 on the Mac. The Tier-3 fallback is `http://127.0.0.1:8092`.
>
> **✅ SUBSTITUTIONS NOW APPLIED IN THE BODY — 2026-07-17 (team_100, M12 W8 · `AOS-V5-M12-WP-L0-ACTOR-KEY-CANON`).**
> This override previously said *"Read every `:8090` in §15.x as `:8092`"* and left the body text unchanged
> *"for v1.x audit-trail continuity"*. **That deferral was the defect, not the fix.** A canon that asks the
> reader to mentally substitute a value will be obeyed **literally** — and it was:
>
> > **On 2026-07-16 a spoke followed §15.4 verbatim, exported a variable nothing reads, aimed it at a port that
> > returns 410, concluded it had no key, declared a false BLOCKER ("Iron Rule #6 is broken for us"), and
> > escalated it to team_00 by hand-carried file believing the mail bus was shut to it. Nothing was broken. Its
> > key existed and authenticated the whole time** (verified live: HTTP 422 = auth passed).
>
> The substitutions are applied in the text now, not delegated to the reader:
>
> | Was (documented) | Now (what the code actually reads) | Authority |
> |---|---|---|
> | `AOS_V3_ACTOR_KEYS` | **`AOS_ACTOR_KEYS`** | `authority.py:40`; renamed in M10-WP1a. **Nothing read the documented name.** |
> | `AOS_V3_TRUST_CLIENT_ACTOR` | **`AOS_TRUST_CLIENT_ACTOR`** | `authority.py:126` |
> | `http://100.125.98.56:8090` | **`http://100.125.98.56:8092`** | `:8090` = retired v4 stub → 410 |
> | `http://127.0.0.1:8090` | **`http://127.0.0.1:8092`** | Tier-3 default |
>
> Remaining `:8090` mentions are **deliberate** — they describe the retired stub's own 410 behaviour and must
> keep naming the retired port to stay meaningful.
>
> **Why this outranked "the docs are stale"** *(team_120 GCR `…ADR043_ACTOR_KEY_DOC_DRIFT_v1.0.0` — ACCEPTED)*:
> finding no per-team key where canon said one would be, the natural workaround is `actor=team_100,
> sender=team_110` — which is **blocked, logged to `security_audit.log`, and escalated to team_00 as an
> impersonation attempt** (`authority.py:220-252`). **A good-faith agent obeying stale canon generates a security
> event.** The canon was manufacturing the incidents it was meant to prevent. It also aimed the spoke at the
> wrong fix: they asked for a clearer 401, but the message was **already correct and test-locked** —
> `INVALID_ACTOR_KEY` (configured team, wrong key) vs `ACTOR_KEY_NOT_CONFIGURED` (no key)
> (`authority.py:93-113`, `test_security_negative_auth.py:97-118`). Without this fix we would have "fixed" a
> correct error string and left the real cause standing.
...
|----------|----------|-------------------------------|---------|
| `AOS_API_BASE` | `msg_preflight.sh` (shell scripts) | Tier-3: `http://127.0.0.1:8092` | Endpoint for pre-flight health probe and all `msg_curl` calls. Highest-priority in the three-tier resolution chain. |
...
| `AOS_ACTOR_API_KEY` | `msg_curl` (conditional) | not set | Shared secret from `AOS_ACTOR_KEYS` for this team. Required when server has `AOS_ACTOR_KEYS` set (all production environments). Injected as `X-Actor-Api-Key`. |
...
```
AOS_API_BASE="${AOS_API_BASE:-${AOS_V3_PUBLIC_API_BASE:-http://127.0.0.1:8092}}"
```
...
| 3 (fallback) | hardcoded default | neither tier 1 nor tier 2 set | `http://127.0.0.1:8092` — correct on waldhomeserver; hits Mac legacy stub (→ HTTP 410) |

### 15.3 Required env vars per session context

| Session context | `AOS_API_BASE` | `AOS_ACTOR_TEAM_ID` | `AOS_ACTOR_API_KEY` | Notes |
|----------------|---------------|---------------------|---------------------|-------|
| Mac Claude Code (team_100) | `http://100.125.98.56:8092` (Tailscale) | `team_100` | from `AOS_ACTOR_KEYS[team_100]` | Set in shell profile or export before sourcing `msg_preflight.sh`. Without this, tier-3 default hits Mac stub → HTTP 410. |
| waldhomeserver team_99 session | `http://127.0.0.1:8092` | `team_99` | from `AOS_ACTOR_KEYS[team_99]` | Tier-3 default IS correct here (waldhomeserver runs the real API on **`:8092`** — corrected 2026-07-17; this cell said `8090`, the retired stub). Add `AOS_API_BASE` to `core/.env` to make explicit. |
| waldhomeserver team_61 session | `http://127.0.0.1:8092` | `team_61` | from `AOS_ACTOR_KEYS[team_61]` | Same as team_99. |
| AOS API server process itself | N/A | N/A | N/A | The process IS the server; it does not call itself. Reads `AOS_V3_DATABASE_URL`, `AOS_ACTOR_KEYS`, `AOS_V3_SERVER_PORT` from `EnvironmentFile` in systemd unit. |

### 15.4 Actor key retrieval and setup (v5)

> **REWRITTEN 2026-07-17** — team_100, M12 W8 (`AOS-V5-M12-WP-L0-ACTOR-KEY-CANON`), per team_120 GCR
> `GOVERNANCE_CHANGE_REQUEST_ADR043_ACTOR_KEY_DOC_DRIFT_v1.0.0` item (c), **ACCEPTED**.
> The previous text described a **manual `.zshrc` export of one key** and **never mentioned the keystore, the
> provisioner, or `aos_actor`** — `grep -rl provision_actor_key governance/` returned **zero files**. Two canons
> gave two answers (`AOS_ACTOR_KEY_PROCEDURE_v1.0.0.md` §0 already documented the provisioner), and the stale one
> is the one cited from `CLAUDE.md:57` and `.claude/commands/AOS_dispatch.md:8`. **That is what produced the
> 2026-07-16 false BLOCKER** (see the §15 override block above).

**The key inventory is the keystore, not your shell profile.**

Keys live in the multi-team keystore **`~/.aos/actor_keys.json`** (mode `0600`), provisioned by
**`scripts/provision_actor_key.sh`** (issuer: **team_00 / team_99 only**).

> ⚠ **`~/.aos/actor.env` is the ACTIVE-SESSION BINDING for ONE team** (default `team_100`). **It is NOT the key
> inventory.** **A team absent from `actor.env` is NOT a team without a key.** Reading `actor.env`, finding your
> team missing, and concluding "I have no key" is exactly the false-blocker path — check the **keystore**.

**To act as another team in a provisioned shell:**
```bash
aos_actor <team_id>          # switches the ACTIVE binding to that team's key from the keystore
```

**To check whether a key is set — never echo the value** (H7, 2026-06-24):
```bash
[ -n "$AOS_ACTOR_API_KEY" ] && echo KEY_SET || echo KEY_NOT_SET
```

**Mac session (one-time), if the keystore is already provisioned:**
```bash
export AOS_API_BASE=http://100.125.98.56:8092     # :8090 is the RETIRED v4 stub -> HTTP 410
export AOS_ACTOR_TEAM_ID=team_100
aos_actor team_100                                 # binds AOS_ACTOR_API_KEY from ~/.aos/actor_keys.json
```
Never commit a key to source control. The server-side variable is **`AOS_ACTOR_KEYS`** (JSON map
`team_id → secret`, read by `authority.py:40`) — **not** `AOS_V3_ACTOR_KEYS`, which nothing reads.

**If you genuinely have no key:** ask **team_00 / team_99** to run `scripts/provision_actor_key.sh` for your
team. Do **NOT** work around it with `actor=team_100, sender=team_<you>` — that is **blocked, logged to
`security_audit.log`, and escalated to team_00 as an impersonation attempt** (`authority.py:220-252`).

**Reading the 401 correctly — the two codes are distinct and test-locked** (`authority.py:93-113`,
`test_security_negative_auth.py:97-118`):

| Code | Means | Do |
|---|---|---|
| `ACTOR_KEY_NOT_CONFIGURED` | the **server** has no key entry for your team | ask team_00/team_99 to provision |
| `INVALID_ACTOR_KEY` | your team **is** configured; **your key is wrong/stale** | re-bind via `aos_actor <team>`; do not request provisioning |

**⚠ An env-cutover can silently reset the server's key set.** A deploy that rewrites server `core/.env` can leave
`AOS_ACTOR_KEYS` team_00-only → **every non-team_00 bus write 401s**. Re-merge the full team set preserving
team_00's existing key, restart, then sync the local keystore **retrieve-only** — never re-provision a key that
was preserved (that rotates it and breaks the other side).

**waldhomeserver session (one-time):** add to shell profile on waldhomeserver:
```bash
export AOS_API_BASE=http://127.0.0.1:8092
export AOS_ACTOR_TEAM_ID=team_99       # or team_61
export AOS_ACTOR_API_KEY=<value from /data/projects/agents-os/core/.env AOS_ACTOR_KEYS>
```

Also add to `/data/projects/agents-os/core/.env` (so `source core/.env` works as tier-2 seed):
```bash
AOS_API_BASE=http://127.0.0.1:8092
```
...
**Auth model.** When the server has `AOS_ACTOR_KEYS` set (all production environments), BOTH `X-Actor-Team-Id` AND `X-Actor-Api-Key` headers are required on authenticated endpoints. When `AOS_TRUST_CLIENT_ACTOR=1` (local dev / test only), `X-Actor-Api-Key` is not enforced.
