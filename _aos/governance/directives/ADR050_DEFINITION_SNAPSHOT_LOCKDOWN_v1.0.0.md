...
### 2.4 def↔gov pairing — Check 13 ORPHAN_GOV stays a blocking FAIL (ratified 2026-06-28, AOS-V5-WP-GOV-SNAPSHOT-HARDENING)

`validate_aos.sh` **Check 13** is the companion of Check 47 on the other axis: where Check 47 compares a spoke's `_aos/definition.yaml` team blocks against the hub, Check 13 checks **def↔gov pairing within a spoke** — every `team_NN:` block in `_aos/definition.yaml` must have a matching `_aos/governance/team_NN.md`, and (bidirectional, WP M9-P1-WP7) every `_aos/governance/team_NN.md` must have a matching def block:

- `MISSING_GOV` — def block with no governance file (ghost team) → **FAIL**.
- `ORPHAN_GOV` — governance file with no def block (dissolved/renamed team left behind, e.g. `team_190.md` after team_190 collapsed into team_90) → **FAIL**.

**Ratified semantics:** ORPHAN_GOV remains a **blocking FAIL** (no downgrade to WARN). The §2.1 "subset" allowance governs *which* teams a spoke materializes — but **def and gov must stay in lockstep** for whatever set the spoke carries. The live v5 propagation model is **full-roster snapshots**: `propagate_governance.sh` copies the full hub team-contract set and `sync_definition_snapshots.sh` carries the full hub roster, so every spoke holds the same teams in both `definition.yaml` and `governance/` (def == gov, both full). Under that model an orphan governance file is genuine drift — a dissolved-team contract the reconcile prune must remove — so the strict bidirectional FAIL is correct, and is exactly what protects against the team_190→team_90 / team_191→team_120 leftovers this WP cleaned up. A *subset* spoke (were one introduced) must carry the matching governance subset; it must never carry a gov file for a team absent from its own `definition.yaml`. The R5 determinism fix (argv-passed paths, single atomic per-process snapshot) makes this verdict stable across fleet sweeps.
