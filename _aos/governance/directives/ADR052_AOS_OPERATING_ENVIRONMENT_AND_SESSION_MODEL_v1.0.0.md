...
**Addendum (M7, ACCEPTED 2026-06-19):** Decision #5 is extended by `ADR052_ADDENDUM_M7_AUTONOMOUS_DISPATCH_DOCTRINE_v1.0.0.md` — "autonomous to the gate, human at the gate" (event-gated server wake; gate-advance / destructive / main-merge / cross-engine stay human; Phase-1 visible-first). Co-signed team_00 + team_100 (IR#12).
**Addendum (Orchestrator Worktree Enforcement, ACCEPTED 2026-07-15):** Decision #2's "worktree-isolated" clause is mechanically enforced for fleet-mutating orchestration by `ADR052_ADDENDUM_ORCHESTRATOR_WORKTREE_ENFORCEMENT_v1.0.0.md` — new Iron Rule #17 (`scripts/lib/aos_worktree_guard.sh` refuse-to-run + `validate_aos.sh` Check 76 dangling-residue detect). team_00 direct approval (IR#12).
**Addendum (Autonomous Merge Policy, ACCEPTED 2026-07-15):** the M7 "Merge to main → KEEP HUMAN" line is refined into a deterministic, machine-checkable policy by `ADR052_ADDENDUM_AUTONOMOUS_MERGE_POLICY_v1.0.0.md` — a `gh pr merge` is auto-allowed only for an OPEN, non-draft, checks-green, allowlisted-branch PR without `--admin`; else ASK (degrade→ASK). Enforced by `scripts/hooks/gh_merge_guard.sh` (PreToolUse) + `validate_aos.sh` Check 77, propagated fleet-wide. team_00 direct approval (IR#12).
