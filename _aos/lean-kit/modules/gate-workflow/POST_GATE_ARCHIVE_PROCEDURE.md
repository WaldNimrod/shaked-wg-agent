...
**Version:** v1.3.0
**Date:** 2026-07-09 (v1.3.0 — verify-and-move invariant added: physical-presence check after move, closes "retained-in-place" bug; was v1.2.0 2026-07-05)
**Authority:** Team 00 + Team 100
**Custodian:** team_120 (procedure + `_aos/` authority, inherited from **dissolved team_191**, D-191auth 2026-07-05)
**Executor:** the closing orchestrator (team_100 hub-native; team_110 under ADR045) via `/AOS_archive` (API-mediated `archive.py`); git commit/push by team_60 — or any team under an explicit Team 00 mandate
...
See **Step 6b** below — do not consider a file archived until its destination is verified.

### Step 6b: Verify Physical Presence (mandatory)

`git mv` (or `mv`) returning success is **not** on its own proof that a file physically
landed at its destination — a WP has previously been marked archived while its files
were actually retained in place (the "retained-in-place" bug). After Step 6, verify every
intended destination from the move plan actually exists on disk:

```bash
# For every {src, dst} pair from Step 6, confirm dst physically exists:
for dst in "${MOVED_DST[@]}"; do
    if [[ ! -e "$dst" ]]; then
        echo "VERIFY FAILED — missing after move: $dst"
    fi
done
```

**Invariant (binding):** an archive is complete ONLY when files are verified physically
present under `_archive/<WP-ID>/` — never merely status-tagged (`status: COMPLETE`,
an ARCHIVE_MANIFEST.md entry, or a `git mv` exit code) as moved. If any destination fails
verification, STOP — do not record the WP as archived; fix the move and re-verify before
proceeding to Step 7.

The API-mediated path (`archive.py` `execute_archive()`) performs this verification
automatically: it returns `intended_count` and `verified_count` alongside `moved`/`errors`,
and only verified (physically-present) paths are counted in `moved` and written into
ARCHIVE_MANIFEST.md — a path that reports `git mv` success but fails verification is
demoted to `errors`, not silently treated as archived.
...
### Step 11 (WP-closure Step 5): Worktree teardown — terminal, best-effort

After the archive is committed, the WP-closure sequence finishes with **Step 5 — worktree teardown** (ADR042 Addendum v1.2.0). If the WP was built in a **dedicated worktree** and is `LOD500_LOCKED` (dual DB/file-canonical gate), the closer removes exactly that worktree via the pinned **single-target** entrypoint:

```bash
AOS_REAP_CONFIRM=YES bash scripts/session_reap.sh --wp-id "$WP_ID" --confirm
```

Best-effort and idempotent: a WP with no dedicated worktree, or any safety-clause failure (dirty tree / a foreign live session / no merge proof), ⇒ **SKIP + WARN** — it NEVER fails closure. Never use the un-scoped batch sweep (`session_reap.sh --confirm`) as the closer. Full runbook: `governance/directives/ADR042_ADDENDUM_WORKTREE_TEARDOWN_v1.2.0.md`.
...
- [ ] **Step 6b:** physical presence of every moved file verified on disk (`verified_count == intended_count`; no destination missing)
...
- [ ] Changes committed
- [ ] **WP-closure Step 5:** dedicated worktree torn down (`session_reap.sh --wp-id`) or SKIPPED (no worktree / not `LOD500_LOCKED`) — best-effort, never blocks closure (ADR042 Addendum v1.2.0)
...
*Team 100 + team_120 (custodian) | Post-Gate Archive Procedure v1.3.0 | 2026-07-09 (verify-and-move invariant) | Iron Rule #15*
