...
date: 2026-04-20
port_registry_version: 2.7.1
ratification: _COMMUNICATION/team_100/RATIFICATION_PORT_CANON_v2.7.0_MINECRAFT_HOME_FORGE_2026-07-21.md
---
...
- **Cross-host reservation (R10 v2.5.0):** within a single host, once a port is assigned to any service of any project in any environment, it is RESERVED for that project on that host — no other project may bind the same port on the same host. Cross-host reuse by different projects is permitted when each assignment is explicitly documented with host scope in `canonical_port_decision` (see port-registry.yaml binding_rules.R10).
...
1. **Port Registry Compliance (v2.2.0)**
   ...
   - Cross-host reservation (R10 v2.5.0): ports are reserved per host from first assignment — same port on different hosts is permitted when host-scoped in `canonical_port_decision`; no global-across-all-hosts ban
...
**Created:** 2026-04-18  
**Last Updated:** 2026-07-21 (MODULE.md R10 human-doc reconciled to host-scoped R10 v2.5.0 — MCF-06; port-registry v2.7.0 minecraft-home-forge)
