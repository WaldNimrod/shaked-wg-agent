...
**Complete AOS command name index (Tier 1 + Tier 2, ADR031):** `/AOS_archive`, `/AOS_decide`, `/AOS_dispatch`, `/AOS_domain-health`, `/AOS_gate-mandate`, `/AOS_gate-status`, `/AOS_gov-sync`, `/AOS_gov-update`, `/AOS_handoff`, `/AOS_help`, `/AOS_mail`, `/AOS_project-init`, `/AOS_qa`, `/AOS_send` *(deprecated → `/AOS_SendMail`)*, `/AOS_SendMail`, `/AOS_server`, `/AOS_validate`.
