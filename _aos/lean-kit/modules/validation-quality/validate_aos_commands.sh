...
# ================================================================
check_a4() {
    local registry_file="$SCRIPT_DIR/GATE_REGISTRY.md"
    if [ ! -f "$registry_file" ]; then
        registry_file="$PROJECT_ROOT/core/modules/validation-quality/GATE_REGISTRY.md"
    fi
    if [ ! -f "$registry_file" ]; then
        registry_file="$PROJECT_ROOT/_aos/lean-kit/modules/validation-quality/GATE_REGISTRY.md"
    fi
    if [ ! -f "$registry_file" ]; then
...
# ================================================================
check_a8() {
    # v5 canonical tree (AOS_V5/). The TikTrack/SmallFarms paths were stale OLD /Documents/<spoke>
    # references left from before the v5 cutover (C1); they pointed at duplicate dirs that team_110's
    # WS3 cleanup has since removed — repointed here to the AOS_V5/ canonicals (matches Check 74).
    local spokes=(
        "/Users/nimrod/Documents/AOS_V5/AOS-Sandbox-Lean"
        "/Users/nimrod/Documents/AOS_V5/AOS-Sandbox-Full"
        "/Users/nimrod/Documents/AOS_V5/TikTrack-Phoenix_AOSProject"
        "/Users/nimrod/Documents/AOS_V5/SmallFarmsAgents"
    )
    local any_spoke=0
    ...
    # Resolve the CANONICAL hub path (the MAIN worktree). Spoke settings.json reference the
    # main hub (e.g. /Users/nimrod/Documents/agents-os) — never a linked worktree path, nor the
    # literal "." default. Using $PROJECT_ROOT directly caused a false PASS (PROJECT_ROOT="." →
    # grep "." matches every line) and a false FAIL when run from a worktree (worktree abs path
    # absent from spoke settings). git --git-common-dir resolves the main worktree from any
    # linked worktree. (AOS-V4.5-WP-GOV-DISTRIBUTION-MODELB / ADR052 worktree workflow.)
    local hub_path cg
    cg="$(git -C "$PROJECT_ROOT" rev-parse --git-common-dir 2>/dev/null || true)"
    case "$cg" in
        /*)  hub_path="$(cd "$(dirname "$cg")" 2>/dev/null && pwd)" ;;
        ?*)  hub_path="$(cd "$PROJECT_ROOT/$(dirname "$cg")" 2>/dev/null && pwd)" ;;
    esac
    [ -n "${hub_path:-}" ] || hub_path="$(cd "$PROJECT_ROOT" 2>/dev/null && pwd)"
    for spoke in "${spokes[@]}"; do
