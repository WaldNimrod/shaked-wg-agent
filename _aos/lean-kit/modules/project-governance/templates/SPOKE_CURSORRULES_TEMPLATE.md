# {{PROJECT_NAME}} — Cursor Rules (AOS canonical)

<!-- AOS-CANONICAL-TEMPLATE v1.0.0 — rendered by scripts/aos_sync_all.sh. DO NOT hand-edit content between aos:canonical markers. -->

<!-- aos:canonical:start -->
## ⚠ AOS Spoke Notice (READ FIRST)

You are working inside an **AOS spoke** (`{{REPO_NAME}}`, profile `{{PROFILE}}`). AOS is a multi-domain, multi-engine infrastructure for managing agents and projects across the organization. This repo's `_aos/` directory is a READ-ONLY snapshot of hub governance at `{{HUB_PATH}}`. Do NOT edit `_aos/` directly. To request changes, file a `GOVERNANCE_CHANGE_REQUEST` in `_COMMUNICATION/team_XX/` → route to `team_100`.
...
## Mandatory Session Startup (canonical uniform order)
...
4. **[CTX-05] DB/API health probe (mandatory — LIVE):** `_api="${AOS_API_BASE:-http://100.125.98.56:8092}"; curl -s --max-time 3 "$_api/api/system/health"` then parse JSON `db.status`:
   - `online` → API + DB online; structured mutations via API (Iron Rule #7 / ADR034).
   - `offline` **or** HTTP **410** → **STOP**: report to Team 00, wait for Team 00 guidance (ADR034 R8 on a named branch — never main).
   - HTTP **000** → API unreachable (check Tailscale connectivity to the v5 server).
   - *Degraded fallback only (may be stale):* `cat "{{HUB_PATH}}/_aos/db_connectivity_status.json"` — hub-written file; use ONLY if the live curl cannot run. The fallback URL is the **:8092** v5 server — never `:8090` (v3/Mac-stub port).
...
## Iron Rules (IMMUTABLE — uniform across all AOS domains)

1. **Cross-engine validation:** builder engine ≠ validator engine
2. **Physical lean-kit / governance cache:** `_aos/lean-kit/` (and governance snapshots) are a physical copy or a git-ignored local cache (Model B / ADR054), never a symlink
3. **Repo-internal refs:** `spec_ref` paths are always repo-internal
4. **Roadmap single-writer:** one agent holds write authority at a time
5. **L-GATE_VALIDATE independence:** DUAL gate — `team_90` (governance facet) + `team_50` (functional facet), both must PASS; constitutional, cross-engine, immutable
6. **Inter-team = artifact:** communication via canonical file in `_COMMUNICATION/`
7. **Data authority:** when AOS v5 DB online, structured mutations via API + `deploy_cascade()` only (ADR034)
8. **Port canon:** long-running listeners registered in `lean-kit/modules/12-home-server-infrastructure/deployment/port-registry.yaml` (Team 60 SSOT)
...
- **This repo is:** `{{REPO_NAME}}` ({{PROFILE}} spoke)
- **This repo is NOT:** the AOS hub, any other spoke
...
`HUB_API_BASE` defaults to `http://127.0.0.1:8092` for local dev; override via `AOS_API_BASE` env var when hub runs on waldhomeserver (V321).
...
## Working with Cursor / Environment Capabilities

Spoke agents running in Cursor (or any IDE engine) MUST read the environment-capabilities catalog before assuming tool availability, cloud vs local lanes, or gateway paths:

- Hub SSoT: `{{HUB_PATH}}/methodology/AOS_ENVIRONMENT_CAPABILITIES_AND_CURSOR_v1.0.0.md`
- Spoke snapshot (when present): `_aos/methodology/AOS_ENVIRONMENT_CAPABILITIES_AND_CURSOR_v1.0.0.md`
