...
my-project/
  ...
  _COMMUNICATION/             # Inter-team artifact exchange
    ...
    team_90/                  # Validator Agent (team_190 retired → team_90)
...
mkdir -p _COMMUNICATION/{team_00,team_100,team_110,team_90}
...
# Module 12 (managed-pipeline / L2.5) retired as of v4.0.0 -- see MANAGED track (ADR044 §4)
```
...
**Rule:** include ONLY teams that appear in `team_assignments.yaml`. The template pre-fills the 3 always-required teams (team_00, team_100, team_90) and team_110 (domain builder). Remove any other team entry that is not assigned. (`team_190` is retired — validator slot is `team_90`.)
...
2. **Physical lean-kit / governance cache:** `_aos/lean-kit/` (and governance snapshots) is a physical copy or a git-ignored local cache refreshed by sync (Model B / ADR054), never a symlink
