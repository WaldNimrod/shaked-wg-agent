...
#   Tier 2: AOS_PUBLIC_API_BASE (from core/.env — canonical server URL for Python + shell)
#   Tier 3: localhost fallback (correct on waldhomeserver; hits stub on Mac → HTTP 410)
AOS_API_BASE="${AOS_API_BASE:-${AOS_PUBLIC_API_BASE:-http://127.0.0.1:8090}}"
...
_probe_api() {
  ...
  if [ "$http_code" = "410" ]; then
    # A Mac legacy stub (the v4.0.0 GA aos_legacy_stub.py, removed in the v5.1.1 cleanup) served this.
    ...
    if [ "$_MSG_PREFLIGHT_VERBOSE" -eq 1 ]; then
      ...
      printf '       or set AOS_PUBLIC_API_BASE=http://100.125.98.56:8090 in core/.env\n'
...
# BEGIN GENERATED FROM _aos/projects.yaml — DO NOT EDIT BY HAND
  ...
  case "$remote_url" in
    ...
    *famely-neusletter*|*family-newsletter*) printf 'family-newsletter\n' ;;
    *HobbitHome*|*hobbithome*) printf 'hobbithome\n' ;;
    *IsraelMicrogreens-BlenderV2-Project*|*microgreens*|*vertical-hydroponics-pilot*) printf 'microgreens\n' ;;
    *nimrod-bio*|*nimrod.bio*) printf 'nimrod-bio\n' ;;
    *nimrod-book*) printf 'nimrod-book\n' ;;
    *Sadot*|*sadot*) printf 'sadot\n' ;;
...
_emit_auth_fallback_warning() {
  ...
  case "$code" in
    INVALID_ACTOR_KEY)
      ...
      printf '  Fix:   export the %s secret from AOS_ACTOR_KEYS per ADR043 §15.4, then re-source msg_preflight.sh.\n' "$team_id" >&2
      ;;
    ACTOR_KEY_NOT_CONFIGURED)
      # Server keystore genuinely lacks an entry for this team.
      printf '  Cause: server AOS_ACTOR_KEYS has no entry for %s.\n' "$team_id" >&2
      printf '  Fix:   team_00 adds %s to AOS_ACTOR_KEYS on waldhomeserver core/.env, then restart aos-api (ADR043 §15.4 / §16).\n' "$team_id" >&2
      ;;
    *)
...
# Auth:    Set AOS_ACTOR_API_KEY env var (shared secret configured in AOS_ACTOR_KEYS
#          on the server) to authenticate. Required when server has AOS_ACTOR_KEYS
#          set (production). Not required in local dev (AOS_TRUST_CLIENT_ACTOR=1).
...
msg_curl() {
  ...
  # Inject API key when configured (AOS_ACTOR_KEYS production auth model — SEC-001)
