...
# ── AC-13: No forbidden placeholder markers in deliverables ──────────────────
# The regex below uses bracket-expressions like `[T]BD` so that this file does
# not contain the literal substrings AC-13 forbids; a single-element character
# class matches its sole element, so real occurrences are still found.
echo "AC-13: forbidden-marker scan in deliverables"
marker_found=0
for f in \
    "$HOOK_SCRIPT" \
    ...
    if [ -f "$f" ] && grep -qiE '\b[T]BD\b|\b[F]IXME\b|to be d[e]fined' "$f" 2>/dev/null; then
        echo "  WARN: forbidden marker in: $f"
        marker_found=$((marker_found + 1))
    fi
done
if [ "$marker_found" = "0" ]; then
    pass "0 forbidden markers found"
else
    fail "forbidden markers in deliverables" "$marker_found occurrences"
fi
