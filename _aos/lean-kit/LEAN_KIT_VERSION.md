...
| 3.4.0 | 2026-07-01 | **Doc-completion templates (AOS-V5-M10-W-b / GCR-120-02, GCR-120-04):** two new `document-lifecycle/templates/` files — `DOC_CANON.md.template` (spoke-bootstrap Documentation-Canon; body carries the four Check-59 sections so a spoke instantiation passes `validate_aos` Check 59) and `ASBUILT_LOD500_AUTHORING_RUNBOOK_v5.md.template` (≤2pp backfill-LOD500 runbook; forbids retro LOD300/400 per `LOD_STANDARD_v5` §5.6). Shipped fleet-wide by `aos_sync_all.sh` Step 2 (whole-tree rsync). |
