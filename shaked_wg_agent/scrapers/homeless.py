...
Pagination: /rent/N (simple path-based pages)
...
class HomelessScraper(BaseScraper):
    ...
    def fetch_listings(self) -> list[ScrapedListing]:
        try:
            ...
            logger.warning("playwright not installed — homeless scraper disabled")
            return []
        ...
        try:
            ...
            logger.exception("homeless scraper failed — returning %d partial results", len(results))
        return results

    def _page_url(self, page_num: int) -> str:
        """Build URL for a given page number.

        Homeless.co.il pagination: /rent/N for the base board,
        or /rent/inumber1=41,N for filtered views (comma-separated page).
        Alternatively, some filter combos use /rent/inumber1=41$$page=N.
        We use the simple path-append pattern which works for both cases.
        If page 2 returns identical results to page 1, pagination stops
        because _has_next_page checks the paging div links.
        """
        if page_num == 1:
            return self.search_url
        # For filtered URLs like /rent/inumber1=41, append ,N
        # For base /rent/, append /N
        base = self.search_url.rstrip("/")
        if "inumber" in base or "=" in base:
    ...
    def _parse_row(self, row: Any) -> ScrapedListing | None:
        ...
        # Table columns (0-indexed): [0]=checkbox, [1]=image, [2]=property_type,
        # [3]=city, [4]=neighborhood, [5]=street, [6]=rooms, [7]=floor,
        # [8]=price, [9]=entry_date, [10]=update_date
