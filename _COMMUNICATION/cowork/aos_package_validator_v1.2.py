...
"""
═══════════════════════════════════════════════════════════════════
  AOS Package Validator v1.2
  Generic validation tool for Cowork development packages
  v1.2: Fixed SOURCE_ROOT_CONSISTENCY counting bug, added SHELL_SYNTAX check
═══════════════════════════════════════════════════════════════════
...
class PackageValidator:
    ...
    def check_source_root_definition(self) -> CheckResult:
        ...
        for file_label, val in definitions_found:
            if not val.startswith("/") and not val.startswith("$"):
                ...
                if prompt_rel:
                    ...
                    if prompt_path.exists():
                        ...
                        if not has_export:
                            ...
                            export_check_resolved = True
                            break
                        else:
                            # Export found — relative path is properly resolved via shell variable
                            result.passed += 1
                            self._pass()
                            export_check_resolved = True
                            break
                ...
                export_check_resolved = True
                break
        if not export_check_resolved and not definitions_found:
            pass  # already flagged above as SOURCE_ROOT_UNDEFINED
    ...
    # ── CHECK 10b: Shell Syntax Safety ──────────────────────────

    def check_shell_syntax(self) -> CheckResult:
        """Check for common shell syntax pitfalls in verification gate commands.

        Detects:
        - BRE quoting issues: \\( inside double quotes becomes a BRE group opener
        - Unescaped special chars in grep patterns
        - Mixed quoting that may cause unexpected behavior
        """
        result = CheckResult(category="SHELL_SYNTAX")
        ...
        for i, line in enumerate(lines, 1):
            ...
            # BRE quoting check: \( inside double quotes is a BRE group opener
            # Pattern: grep ... "...\(..." — the \( inside double quotes is NOT
            # escaped to a literal paren; it opens a BRE capturing group.
            # Correct: use single quotes or -E (ERE mode)
            dq_segments = re.findall(r'"([^"]*)"', stripped)
            bre_issue = False
            for seg in dq_segments:
                # \( inside double-quoted string in grep (not -E mode)
                if '\\(' in seg and '-E' not in stripped and '--perl-regexp' not in stripped:
                    bre_issue = True
                    break
            ...
            if bre_issue:
                ...
                self._fail()
                self._add_finding(
                    check_id="SHELL_BRE_QUOTING",
                    severity="HIGH",
                    category="COMMAND",
                    title=f"BRE quoting issue: \\( in double quotes opens a group",
                    detail=f"Line {i}: {stripped[:120]}",
                    file=str(fpath.relative_to(self.pkg_root)),
                    line=i,
                    suggestion="Use single quotes for the pattern (e.g., grep -v '\\.get('), or use -E for Extended RE."
                )
            else:
                result.passed += 1
                self._pass()
    ...
    # ── CHECK 11: Mount Path Alignment ──────────────────────────

    def check_mount_path_alignment(self) -> CheckResult:
        """Verify SOURCE_ROOT path contains the package folder name."""
        result = CheckResult(category="MOUNT_PATH_ALIGNMENT")

        # Extract folder name from package_root
        pkg_folder = Path(self.config["package_root"]).name

        # Find SOURCE_ROOT definitions in prompt and instructions
        files_to_scan = []
        for key in ["instruction_file", "prompt_file"]:
            rel = self.config.get(key, "")
            if rel:
        ...
        for file_label, fpath in files_to_scan:
            ...
            # Find SOURCE_ROOT value definitions
            defs = re.findall(
                rf'{re.escape(label)}\s*=\s*["\']?([^"\'\n]+)["\']?',
                content
            )
            for d in defs:
                val = d.strip()
                result.total_checks += 1
                # Check if the defined path contains the package folder name
                if pkg_folder in val:
                    result.passed += 1
                    self._pass()
                else:
                    result.failed += 1
                    self._fail()
                    self._add_finding(
                        check_id="MOUNT_PATH_MISMATCH",
                        severity="CRITICAL",
                        category="PATH",
                        title=f"{label} path does not match package folder name",
                        detail=f"{label}=\"{val}\" but package folder is \"{pkg_folder}\". In Cowork, mounted folder name becomes part of the path.",
                        file=str(fpath.relative_to(self.pkg_root)),
                        suggestion=f"Change to: {label} = \"mnt/{pkg_folder}/{self.config.get('source_root_rel', 'assets')}\""
                    )
    ...
    def validate(self) -> ValidationReport:
        ...
        checks = [
            ...
            self.check_mount_path_alignment,
        ]
